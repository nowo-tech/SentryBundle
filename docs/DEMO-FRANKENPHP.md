...
The entrypoint, when `APP_ENV=dev`, runs `cp /etc/frankenphp/Caddyfile.dev /etc/frankenphp/Caddyfile` before starting FrankenPHP. Default port is **8001** (see each demo’s `.env` or `PORT`). Optional: set `SENTRY_DSN` in the environment if you want to send events to Sentry.

### 5. Entrypoint (development-friendly)

The demos’ entrypoint creates `var/cache` and `var/log`, and when `APP_ENV=dev` overwrites the Caddyfile with the dev version (no worker). If vendor is not present, it runs `composer install`. The **Makefile** (`make up`) runs `composer install` after starting the stack. No cache clear is performed inside the container on each start; run `php bin/console cache:clear` from the demo directory (or `make -C demo/symfony8 shell` then `php bin/console cache:clear`) if needed.
...
- Clear the Symfony cache: `docker-compose exec php php bin/console cache:clear` or from the demo directory `make shell` then `php bin/console cache:clear`.

### Demo does not respond or "make up" times out

- The Makefile runs `composer install` after starting the stack. Ensure the port in `.env` (e.g. `PORT=8001`) is free and that the container starts (check `docker-compose logs php`).
- If the container exits, check that the entrypoint and Caddyfile are valid and that required env vars (e.g. `APP_SECRET`, `SENTRY_DSN` if you use Sentry) are set.
