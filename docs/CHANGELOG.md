...
- **Code style**: PHP-CS-Fixer follow-up on `SentryErrorReporter` (formatting only; merged via automated PR).
- **Demos**: Refreshed `composer.lock` for Symfony 7 and Symfony 8 demo apps; updated `demo/symfony8/config/reference.php` from the framework recipe.
